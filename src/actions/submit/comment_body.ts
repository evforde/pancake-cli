...
abstract class StackCommentBodyBase {
  ...
  protected buildTreeComment(currentPr: PR): string[] {
    ...
    const buildLine = (pr: PR | Pick<PR, 'base' | 'ref'>, children: Array<PR | Pick<PR, 'base' | 'ref'>>) => {
      if ('number' in pr) {
        ...
        if (children.length > 1) {
          ...
          line += ` Dependent PRs: (${children.map(pr => buildShortLine(pr)).join(', ')})`;
        }
    ...
    while (current) {
      const children: Array<PR | Pick<PR, 'base' | 'ref'>> = this.tree[current.ref];
    ...
    while (currentRef !== trunk) {
      const pr: PR | Pick<PR, 'base' | 'ref'> = this.prsByBranchName[currentRef];
      const childrenOfPr: Array<PR | Pick<PR, 'base' | 'ref'>> = this.tree[currentRef];
